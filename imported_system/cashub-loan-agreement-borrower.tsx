  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4 md:p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {currentScreen === 2 && (
          <div className="bg-white/10 backdrop-blur-lg rounded-xl shadow-xl border border-white/20 p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              >
                <Edit className="mx-auto mb-3 text-purple-300" size={32} />
                <p className="text-white font-medium mb-1">Draw Signature</p>
                <p className="text-purple-200 text-sm">Sign with your finger or mouse</p>
              </button>
              ...
              >
                <Smartphone className="mx-auto mb-3 text-purple-300" size={32} />
                <p className="text-white font-medium mb-1">OTP Verification</p>
                <p className="text-purple-200 text-sm">Verify via phone/email</p>
              </button>

              <button
                onClick={() => setSignatureMethod('selfie')}
                className={`p-6 rounded-xl border-2 transition-all ${
              ...
              >
                ...
                <p className="text-white font-medium mb-1">Selfie + Accept</p>
                <p className="text-purple-200 text-sm">Take a selfie to confirm</p>
              </button>
            </div>
            ...
            {signatureMethod === 'draw' && (
              <div className="bg-white/5 rounded-lg p-6 border border-white/10 mb-6">
                <h3 className="text-white font-semibold mb-4">Draw Your Signature</h3>
                <div className="bg-slate-800 rounded-lg border-2 border-dashed border-white/20 mb-4">
                  <canvas
                    ref={canvasRef}
                    width={600}
                    height={200}
            ...
            {signatureMethod === 'otp' && (
              <div className="bg-white/5 rounded-lg p-6 border border-white/10 mb-6">
                <h3 className="text-white font-semibold mb-4">OTP Verification</h3>
                <p className="text-purple-200 text-sm mb-4">
                  We'll send a 6-digit code to your phone ({borrowerData.phone}) and email ({borrowerData.email})
                </p>
                <button
                  onClick={sendOTP}
                  className="mb-4 px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-white text-sm font-medium transition-all"
                >
                  Send OTP
                </button>
                <div>
                  <label className="block text-sm font-medium text-purple-200 mb-2">Enter 6-Digit Code</label>
                  <input
                    type="text"
                    maxLength={6}
                    value={otpCode}
                    ...
                    placeholder="000000"
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white text-center text-2xl tracking-widest focus:ring-2 focus:ring-purple-400"
                  />
                </div>
              </div>
            )}
            ...
            {signatureMethod === 'selfie' && (
              <div className="bg-white/5 rounded-lg p-6 border border-white/10 mb-6">
                <h3 className="text-white font-semibold mb-4">Take a Selfie</h3>
                <div className="bg-slate-800 rounded-lg border-2 border-dashed border-white/20 aspect-video flex items-center justify-center mb-4">
                  <Camera className="text-purple-300" size={64} />
                </div>
                <button className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-white text-sm font-medium transition-all">
                  Take Photo
                </button>
              </div>
            )}
            ...
            <div className="bg-white/5 rounded-lg p-4 border border-white/10 mb-6">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-purple-200">Signing as</p>
                  <p className="text-white font-medium">{borrowerData.firstName} {borrowerData.lastName}</p>
                </div>
                <div>
                  <p className="text-purple-200">Date & Time</p>
                  <p className="text-white font-medium">{new Date().toLocaleDateString()} {new Date().toLocaleTimeString()}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 rounded-lg text-white font-semibold transition-all hover:scale-105 flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Lock size={20} />
                Sign Agreement
              </button>
        ...
        {currentScreen === 6 && (
          <div className="bg-white/10 backdrop-blur-lg rounded-xl shadow-xl border border-white/20 p-8 text-center">
            <div className="w-24 h-24 bg-gradient-to-br from-green-500 to-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6">
              <CheckCircle className="text-white" size={48} />
            </div>
            <h2 className="text-3xl font-bold text-white mb-4">Agreement Signed Successfully!</h2>
            <p className="text-purple-200 text-lg mb-8">Your loan is now pending disbursement</p>

            <div className="bg-white/5 rounded-lg p-6 border border-white/10 mb-8 text-left">
              <h3 className="text-white font-semibold mb-4">What Happens Next?</h3>
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <CheckCircle className="text-green-400 flex-shrink-0 mt-0.5" size={20} />
                  <p className="text-purple-200 text-sm">Lender will review and countersign the agreement</p>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle className="text-green-400 flex-shrink-0 mt-0.5" size={20} />
                  <p className="text-purple-200 text-sm">Funds will be disbursed to your account within 24-48 hours</p>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle className="text-green-400 flex-shrink-0 mt-0.5" size={20} />
                  <p className="text-purple-200 text-sm">You'll receive a confirmation SMS and email</p>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle className="text-green-400 flex-shrink-0 mt-0.5" size={20} />
                  <p className="text-purple-200 text-sm">First payment due on {loanDetails.firstPaymentDate}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 mb-8">
              <div className="flex gap-3">
                ...
                <div className="text-blue-200 text-sm text-left">
                  <p className="font-semibold mb-1">Cooling-Off Period Reminder</p>
                  <p>You have 5 business days to cancel this agreement without penalty if you change your mind.</p>
                </div>
              </div>
            </div>

            <div className="flex gap-3">
              <button className="flex-1 px-6 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg text-white font-medium transition-all flex items-center justify-center gap-2">
                <Download size={20} />
                Download Agreement PDF
              </button>
              <button className="flex-1 px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 rounded-lg text-white font-medium transition-all hover:scale-105 flex items-center justify-center gap-2 shadow-lg">
                <Eye size={20} />
...
export default LoanAgreementBorrower; Personal Details */}
