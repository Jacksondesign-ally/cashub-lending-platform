...
import {
  ...
  Clock, XCircle, Pen, Upload, Trash2
} from 'lucide-react'
...
export default function LenderContractPage() {
  ...
  const [accepted_authority, setAcceptedAuthority] = useState(false)
  const [accepted_debit, setAcceptedDebit] = useState(false)
  ...
  const clearCanvas = () => {
    ...
    setHasSignature(false); setSigUrl('')
  }

  const canSubmit = accepted_terms && accepted_authority && accepted_debit &&
    form.legal_name && form.signatory_name &&
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        accepted_terms, accepted_authority, accepted_debit,
        status: 'pending',
        agreement_version: 'v1.0',
      }
      if (existingContractId) {
        await supabase.from('lender_contracts').update({ ...payload, updated_at: new Date().toISOString() }).eq('id', existingContractId)
      } else {
        const { data } = await supabase.from('lender_contracts').insert(payload).select('id').maybeSingle()
        if (data?.id) setExistingContractId(data.id)
      }
      setContractStatus('pending')
    } catch (e: any) { alert('Error submitting: ' + e.message) }
    setSaving(false)
  }
      ...
      <div className="max-w-4xl mx-auto my-6 px-4 pb-16" id="contractDocument">
        ...
        <Section n="2" title="Agreement Summary">
          <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-5 text-sm text-neutral-700 space-y-3 leading-relaxed">
            <p>This Agreement is entered into between <strong>Xtreme Group Empire, herein as CasHuB</strong> (&quot;CasHuB&quot;), and the subscribing lending institution (&quot;Lender&quot;). CasHuB is a technology platform provider and shared lending ecosystem administrator. CasHuB does not originate loans, approve loans on behalf of lenders, or assume responsibility for the lender&apos;s independent lending decisions.</p>
        ...
        {/* Section 3 – Subscription Acceptance */}
        <Section n="3" title="Subscription & Auto-Renewal Acceptance">
          <div className="space-y-3">
            <CheckRow checked={accepted_terms} onChange={setAcceptedTerms}
            ...
            <CheckRow checked={accepted_authority} onChange={setAcceptedAuthority}
        ...
        <Section n="4" title="Bank Authorization (Debit Order / Subscription Collection Consent)">
          <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-5 text-sm text-neutral-700 space-y-3 leading-relaxed mb-5">
            <p>By completing this section, the Lender authorizes <strong>Xtreme Group Empire, herein as CasHuB</strong>, or its designated payment processor, to debit the lender&apos;s nominated bank account for subscription fees, renewal fees, onboarding fees (if applicable), and any other approved platform charges under this agreement, subject to the selected package and invoicing terms.</p>
            <p>This authority remains valid for the duration of the agreement and any renewal period, unless cancelled in writing in accordance with the agreement and subject to settlement of outstanding obligations.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="mt-4">
            <CheckRow checked={accepted_debit} onChange={setAcceptedDebit}
        ...
        <Section n="6" title="Digital Signature & Declaration">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
            ...
            <Field label="Date" type="date" value={new Date().toISOString().split('T')[0]} onChange={() => {}} readOnly />
          ...
          {signMethod === 'type' && (
            <div className="no-print">
              ...
              <input type="text" value={typedSig} onChange={e => { setTypedSig(e.target.value); setHasSignature(e.target.value.trim().length > 2) }}
                placeholder="Type your full name here..."
                className="w-full px-4 py-3 border-2 border-neutral-300 rounded-xl text-lg font-['cursive'] italic text-neutral-800 focus:ring-2 focus:ring-red-400 focus:border-red-400" />
            </div>
          )}

          {/* Signature preview for print */}
          {(hasSignature || typedSig) && (
            <div className="mt-4 border border-neutral-200 rounded-xl p-4">
              <p className="text-xs text-neutral-500 mb-2">Signature preview:</p>
              {signMethod === 'draw' && canvasRef.current ? (
                <img src={canvasRef.current.toDataURL()} alt="Signature" className="h-14 object-contain border-b border-neutral-300" />
              ) : (
                <p className="text-xl font-['cursive'] italic border-b border-neutral-300 pb-1 text-neutral-800">{typedSig}</p>
              )}
              <p className="text-[10px] text-neutral-400 mt-1">{form.signatory_name} — {new Date().toLocaleDateString('en-NA', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
            </div>
          )}
        </Section>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mt-6 text-xs text-amber-800">
          <strong>Legal Notice:</strong> This agreement is a professional platform participation contract. Before live use, it should be reviewed by a Namibian legal professional, especially regarding bank debit authorization, auto-renewal, data-sharing, and borrower registry clauses.
        </div>
        ...
        <div className="no-print mt-8 flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-neutral-400 text-center">By submitting, you digitally sign this agreement. Your portal access will be activated after admin approval.</p>
        </div>
      </div>
    </div>
  )
...
function CheckRow({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <label className="flex items-start gap-3 cursor-pointer group">
      <div className={`mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${checked ? 'bg-red-700 border-red-700' : 'border-neutral-300 group-hover:border-red-400'}`}
