...
const inconsistencies: AuditItem[] = [
  ...
  { id: 'INC-06', title: 'Dashboard Layout References Removed Roles', issue: 'Handles lender, admin, viewer roles which were removed from the system.', fix: 'Clean up to only handle super_admin, lender_admin, loan_officer, borrower.', status: 'pending' },
  { id: 'INC-07', title: 'Scam Alerts Fetches All Lenders\' Alerts', issue: 'Fetch has no submitted_by filter — all scam alerts from every lender are displayed.', fix: 'Add .eq(\'submitted_by\', lenderEmail) for the "my submissions" view.', status: 'pending' },
  ...
  { id: 'INC-09', title: 'Borrower Avatar Stored in localStorage', issue: 'Profile photos stored as base64 in localStorage — device-specific, won\'t persist across browsers, counts toward 5MB limit.', fix: 'Upload to Supabase Storage, store URL in borrower profile.', status: 'pending' },
  ...
  { id: 'INC-14', title: 'Silent Error Handling (Empty Catch Blocks)', issue: '20+ instances of catch {} or catch { setX([]) } with no error logging or user feedback.', fix: 'At minimum, console.error all caught errors. Show toast notifications for user-facing failures.', status: 'pending' },
