...
const suggestions: AuditItem[] = [
  ...
  { id: 'S02', title: 'Move to Supabase Storage for All Files', issue: 'Base64 blobs in DB columns bloat storage by 10-100x.', fix: 'Replace base64 storage for logos, documents, avatars, signatures with Supabase Storage buckets.', status: 'documented' },
  { id: 'S03', title: 'Real-Time Notifications via Supabase Realtime', issue: 'No live updates — users must refresh to see changes.', fix: 'Subscribe to loan_applications, loans, payments, blacklist table changes for instant push notifications.', status: 'documented' },
  { id: 'S04', title: 'Automated Credit Scoring Engine', issue: 'Credit scores are static values with no dynamic calculation.', fix: 'Build scoring algorithm that adjusts based on repayment history, defaults, active debt ratio.', status: 'fixed' },
  { id: 'S05', title: 'Smart Loan Matching', issue: 'Marketplace matching is manual.', fix: 'Auto-match loan requests with lenders based on amount, risk level, approval rate, geography.', status: 'documented' },
  { id: 'S06', title: 'Payment Reminders & Scheduling', issue: 'No automated payment reminders.', fix: 'SMS/email reminders 3 days before due. Escalation alerts for 7/14/30-day overdue.', status: 'documented' },
  { id: 'S07', title: 'Comprehensive Audit Trail', issue: 'Critical actions are not logged consistently.', fix: 'Log all approvals, rejections, blacklist actions, payments, settings changes to audit_logs table.', status: 'fixed' },
  { id: 'S08', title: 'Split Borrower Portal into Sub-Routes', issue: '1549-line single file is unmaintainable.', fix: 'Refactor into app/borrower/loans, apply, marketplace, documents sub-pages.', status: 'documented' },
  { id: 'S09', title: 'Server-Side Data Fetching for Sensitive Queries', issue: 'All queries run client-side, exposing logic and keys in the browser bundle.', fix: 'Move critical queries to Next.js Server Actions or API routes.', status: 'documented' },
  { id: 'S10', title: 'Centralized State Management', issue: 'Scattered localStorage.getItem() calls across 30+ pages — no single source of truth.', fix: 'Replace with React Context or Zustand store for user info, lender ID, company details.', status: 'fixed' },
  { id: 'S11', title: 'Form Validation with Zod', issue: 'No schema validation before database inserts.', fix: 'Add Zod schema validation on all forms — loan applications, registration, blacklist, staff invites.', status: 'fixed' },
  { id: 'S12', title: 'Interest Rate Compliance Auto-Check', issue: 'Loans can be created with any interest rate including non-compliant ones.', fix: 'Auto-flag loans where rates exceed NAMFISA regulatory caps. Block non-compliant loan creation.', status: 'fixed' },
  { id: 'S13', title: 'Visual Analytics with Charts', issue: 'Stats shown as static numbers with no trend visualization.', fix: 'Add Recharts for disbursement volume, collection trends, risk distribution, revenue waterfall.', status: 'fixed' },
  { id: 'S14', title: 'Automated Overdue Detection', issue: 'Loan overdue status must be updated manually.', fix: 'Supabase Edge Function/cron job that checks schedules daily and auto-marks overdue/defaulted loans.', status: 'fixed' },
  { id: 'S15', title: 'Complete Multi-Language (i18n) Support', issue: 'Language dropdown exists but translations are incomplete.', fix: 'Implement next-intl or i18next for English, Afrikaans, and Oshiwambo.', status: 'documented' },
]
